...
import { resolveActivationBaseUrl } from './utils/activationBaseUrl';
...
export const App: React.FC = () => {
  ...
  const activationBaseUrl = resolveActivationBaseUrl();
  ...
  React.useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        const statusUrl = activationBaseUrl
          ? `${activationBaseUrl}/api/activation/status?deviceCode=${encodeURIComponent(deviceCode)}`
          : `/api/activation/status?deviceCode=${encodeURIComponent(deviceCode)}`;
